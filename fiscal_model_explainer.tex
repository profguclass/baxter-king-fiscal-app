...
$G^T_t$ or public investment $I^G_t$. This single fact is the seed of
...
investment $I^G$, and transfers $G^T$. Here $G_B$ is dropped because it
...
model is therefore always exactly two pieces: $G = I^G + G^T$.
\end{notebox}
...
\eq{ \tau_t Y_t = I^G_t + G^T_t, \qquad \tau_t = \frac{I^G_t+G^T_t}{Y_t} }
...
\eq{ \text{Income tax:}\quad C_t+I_t = (1-\tau_t)\big(w_tN_t+r_tK_t\big) + G^T_t }
\eq{ \text{Lump-sum:}\quad C_t+I_t = w_tN_t+r_tK_t - \tau_tY_t + G^T_t }
...
\textbf{Why this is wrong.} Transfers $G^T$ are \emph{not} resource-using:
...
total government spending $G$, public investment $I^G$, transfers $G^T$,
...
\eq{ I^G = f_{I^G}\cdot G, \qquad G^T = f_{G^T}\cdot G \qquad \Longrightarrow \qquad \%\Delta I^G = \%\Delta G^T = \%\Delta G }
\eq{ K^G = \frac{I^G}{\delta} \qquad \Longrightarrow \qquad \%\Delta K^G = \%\Delta I^G }

Chaining these: $\%\Delta K^G=\%\Delta I^G=\%\Delta G=\%\Delta G^T$, always,
...
(Case B). \\[4pt]
Why do $G$, $I^G$, $G^T$, $K^G$ often show identical \% change? &
Because $I^G=f_{I^G}G$, $G^T=f_{G^T}G$, and $K^G=I^G/\delta$ are exact
...
\eq{ \text{Income tax:}\qquad C_t + K_{t+1}-(1-\delta)K_t = (1-\tau_t)\big(w_tN_t+r_tK_t\big)+G^T_t, \label{app:bc-it} }
\eq{ \text{Lump-sum:}\qquad C_t + K_{t+1}-(1-\delta)K_t = w_tN_t+r_tK_t-\tau_tY_t+G^T_t. \label{app:bc-ls} }
...
\textbf{perfect foresight} --- it knows $w_t,r_t,\tau_t,G^T_t$ (and hence
...
\mathcal{L} = \sum_{t=0}^{\infty}\beta^t\Big\{
  ...
  + \lambda_t\Big[(1-\tau_t)(w_tN_t+r_tK_t)+G^T_t-C_t-K_{t+1}+(1-\delta)K_t\Big]
\Big\}.
}
...
\mathcal{L} = \sum_{t=0}^{\infty}\beta^t\Big\{
  \ln C_t + \theta_L\ln(1-N_t)
  + \lambda_t\Big[w_tN_t+r_tK_t-\tau_tY_t+G^T_t-C_t-K_{t+1}+(1-\delta)K_t\Big]
\Big\}.
}
...
Transfers $G^T$ never appear in this identity, because they were never in
...
$N$:
\eq{ Y=\alpha N, \qquad K=\kappa N, \qquad C=s_C\,Y, \qquad I=s_I\,Y, \qquad I^G=s_{I^G}\,Y, \qquad G^T=s_{G^T}\,Y. }
